...
{
    __asm__("mov %%rdi, %%rax;" // MISC: rdi stores the first argument in x86_64 linux calling
                                // convention.
            "lidt (%%rax);"
            "sti"
            :
...
#define KEYBOARD_INTERRUPT_VECTOR 0x21
    idt[KEYBOARD_INTERRUPT_VECTOR].offset_1 =
        (uint16_t)((uint64_t)(&keyboard_isr) & 0x000000000000ffff);
    idt[KEYBOARD_INTERRUPT_VECTOR].offset_2 =
        (uint16_t)(((uint64_t)(&keyboard_isr) & 0x00000000ffff0000) >> 16);
    idt[KEYBOARD_INTERRUPT_VECTOR].offset_3 =
        (uint32_t)(((uint64_t)(&keyboard_isr) & 0xffffffff00000000) >> 32);
